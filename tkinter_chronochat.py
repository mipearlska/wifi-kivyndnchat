...
font10 = "-family KacstPen -size 14 -weight bold -slant roman "
    ...
    def _onData(self, interest, data):
        ...
        if self.getNowMilliseconds() - content.timestamp * 1000.0 < 120000.0:
            ...
            if l == len(self._roster):
                ...
                self._messagelist.insert(tk.END,name + ": Join")
                # print(name + ": Join")
            ...
            if (content.type == chatbuf_pb2.ChatMessage.CHAT and
            	...
            	self._messagelist.insert(tk.END,getattr(content, "from") + ": " + content.data)
                ...
                try:
                    ...
                    if name != self._screenName:
                        self._roster.pop(n)
                        self._messagelist.insert(tk.END,name + ": Leave")
                        # print(name + ": Leave")
                except ValueError:
                    pass
...
def chatwin():
	...
	chatwin = tk.Toplevel(root)
	chatwin.geometry("539x466+460+114")
	chatwin.title(chatRoom.get()+"Channel")

	Label1 = tk.Label(chatwin)
	Label1.place(relx=0.019, rely=0.021, height=18, width=120)
	Label1.configure(font=font11)
	Label1.configure(text='''Welcome, ''' + screenName.get())
	Label1.pack()

	Label2 = tk.Label(chatwin)
	Label2.place(relx=0.019, rely=0.064, height=18, width=120)
	Label2.configure(text='''Channel: ''' + chatRoom.get())
	Label2.pack()

	message_frame = tk.Frame(chatwin)
	message_frame.place(relx=0.019, rely=0.129)
	scrollbar = tk.Scrollbar(message_frame)
	Listbox1 = tk.Listbox(message_frame, height=20, width=20)
	# Listbox1.place(relx=0.019, rely=0.129, relheight=0.652
	#         , relwidth=0.957)
	Listbox1.configure(yscrollcommand=scrollbar.set)
	Listbox1.configure(background="white")
	Listbox1.configure(font="TkFixedFont")
	Listbox1.configure(width=516)
	...
	chat = Chat(
	  screenName.get(), chatRoom.get(), Name(hubPrefix.get()), face, keyChain,
	  keyChain.getDefaultCertificateName(),Listbox1)

	Entry1 = tk.Entry(chatwin)
	Entry1.place(relx=0.019, rely=0.794,height=40, relwidth=0.957)
	Entry1.configure(background="white")
	Entry1.configure(font="TkFixedFont")
	Entry1.configure(width=516)
	Entry1.configure(textvariable=inputmsg)
	Entry1.bind("<Return>", partial(initSendMessageEnter,chat))

	Button1 = tk.Button(chatwin)
	Button1.place(relx=0.807, rely=0.901, height=28, width=89)
	Button1.configure(background="#0e00d8")
	Button1.configure(font=font11)
	Button1.configure(foreground="#ffffff")
	Button1.configure(text='''Send''')
	Button1.configure(width=89)
	Button1.configure(command=partial(initSendMessageButton,chat))

	Button2 = tk.Button(chatwin)
	Button2.place(relx=0.019, rely=0.901, height=28, width=89)
	Button2.configure(background="#d82400")
	Button2.configure(font=font11)
	Button2.configure(foreground="#ffffff")
	Button2.configure(text='''Leave''')
	Button2.configure(width=89)
	Button2.configure(command=partial(chat_leave,chat))

	chatwin.protocol('WM_DELETE_WINDOW', partial(chat_leave,chat))

	chat_start(Listbox1,face,chat)
...
def initSendMessageEnter(chat,event):
	msg = inputmsg.get()
	inputmsg.set("")
	chat.sendMessage(msg)

def initSendMessageButton(chat):
	msg = inputmsg.get()
	inputmsg.set("")
	chat.sendMessage(msg)	

def chat_start(Listbox1,face,chat):
	thread = threading.Thread(target = partial(chatcore,Listbox1,face,chat))
	thread.setDaemon(True)
	thread.start()

def chatcore(Listbox1,face,chat):
...
root.geometry("294x268+436+146")
root.title("NDN Messenger")

screenName = tk.StringVar()
hubPrefix = tk.StringVar()
chatRoom = tk.StringVar()
inputmsg = tk.StringVar()

Label1 = tk.Label(root)
Label1.place(relx=0.31, rely=0.037, height=37, width=112)
Label1.configure(font=font10)
Label1.configure(text='''WELCOME''')
Label1.configure(width=112)

Label2 = tk.Label(root)
Label2.place(relx=0.025, rely=0.299, height=18, width=125)
Label2.configure(text='''Enter Username:''')

Label3 = tk.Label(root)
Label3.place(relx=0.025, rely=0.448, height=18, width=125)
Label3.configure(text='''Enter Channel Prefix:''')

Label4 = tk.Label(root)
Label4.place(relx=0.02, rely=0.597, height=18, width=130)
Label4.configure(text='''Enter Channel Name:''')

Entry1 = tk.Entry(root)
Entry1.place(relx=0.47, rely=0.299,height=20, relwidth=0.503)
Entry1.configure(background="white")
Entry1.configure(font="TkFixedFont")
Entry1.configure(textvariable=screenName)

Entry2 = tk.Entry(root)
Entry2.place(relx=0.47, rely=0.429,height=20, relwidth=0.503)
Entry2.configure(background="white")
Entry2.configure(font="TkFixedFont")
Entry2.configure(textvariable=hubPrefix)

Entry3 = tk.Entry(root)
Entry3.place(relx=0.47, rely=0.578,height=20, relwidth=0.503)
Entry3.configure(background="white")
Entry3.configure(font="TkFixedFont")
Entry3.configure(textvariable=chatRoom)

Button1 = tk.Button(root)
Button1.place(relx=0.379, rely=0.784, height=28, width=63)
Button1.configure(text='''Start!''')
Button1.configure(command = chatwin)
