...
class Chat(object):
    ...
    def _sendInterest(self, syncStates, isRecovery):
        ...
        #check for wrong sequenceInterest sending.
        # Process normally for sequence 0 (error never happens at this sequenceNumber)
        # If sequenceNo got from syncState = previous sequence (sequenceReceived +1): process normally
        # If sequenceNo is different (larger than it should be), resend Interest from expected SequenceNo to the SyncState sequenceNo
        # errorSend = flag. Only resend interest for the first wrong syncState data, other wrong SyncState data will be dropped
        # errorSend will be reset to 0 when all media data are received (see in onData function)
        for i in range(len(sendList)):
            uri = (sendList[i] + "/" + str(sessionNoList[i]) + "/" +
              str(sequenceNoList[i]))
            print "Interestdebug list-received", sessionNoList[i], sequenceNoList[i], self._sequenceReceived
            if self._errorSend == 0:
                if sequenceNoList[i] == 0: 
                    self._previous = sequenceNoList[i]
                ...
                elif sequenceNoList[i] != self._previous:
                    if sequenceNoList[i] == self._sequenceReceived +1:
                        self._previous = sequenceNoList[i]
                        self._sendprefix = sendList[i]
                        self._sessionNoprefix = str(sessionNoList[i])
                        interest = Interest(Name(uri))
                        interest.setInterestLifetimeMilliseconds(self._syncLifetime)
                        self._face.expressInterest(interest, self._onData, self._chatTimeout)
                        print interest.getName(), self._sessionNoprefix
                    else:
                        print self._sessionNoprefix
                        if str(sessionNoList[i]) == self._sessionNoprefix:
                            self._errorSend = 1
                            for k in range (self._sequenceReceived+2, sequenceNoList[i]+1):
                                uri = (self._sendprefix + "/" + self._sessionNoprefix + "/" +str(k))
                                interest = Interest(Name(uri))
                                interest.setInterestLifetimeMilliseconds(self._syncLifetime)
                                self._face.expressInterest(interest, self._onData, self._chatTimeout)
                                self._previous = k
    ...
    def _onData(self, interest, data):
        ...
        """
        # TODO: Check if this works in Python 3.
        sequenceNumber = int(interest.getName().get(self._chatPrefix.size() + 1).toEscapedString())
        self._sequenceReceived = sequenceNumber
        content = chatbuf_pb2.ChatMessage()
    ...
    def _chatTimeout(interest):
        ...
        print "Timeout waiting for chat data", interest.getName()
        # sequenceNumber = int(interest.getName().get(self._chatPrefix.size() + 1).toEscapedString())
        # self._sequenceReceived = sequenceNumber
